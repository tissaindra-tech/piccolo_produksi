...
function HistoryBelanjaView({ belanja, showToast, loadData }) {
  ...
  if (selected) {
    ...
    return (
      <div>
        <div style={{ display: 'flex', gap: '8px', marginBottom: '14px' }}>
          <button onClick={() => { setSelected(null); setEditing(false) }} style={{ ...S.btn, ...S.btnSecondary, fontSize: '12px', padding: '7px 12px' }}>← Kembali</button>
          {!editing && (
            isNotaEditable(selected.tanggal)
              ? <button onClick={() => startEdit(selected)} style={{ ...S.btn, fontSize: '12px', padding: '7px 12px', background: C.yellowBg, color: C.yellow, border: `1px solid ${C.yellowBorder}` }}>✏️ Edit nota ini</button>
              : <div style={{ fontSize: '11px', color: C.text3, padding: '7px 10px', background: C.panel2, borderRadius: '7px' }}>🔒 Nota lebih dari 30 hari — tidak bisa diedit</div>
          )}
        </div>
