...
function Login({ onLogin }) {
  const [users, setUsers] = useState(null)        // null = masih memuat
...
// =====================================================
// KELOLA USER (Owner) — atur staff & PIN dari dalam app
// =====================================================
const AVATAR_OPTIONS = ['🧑‍🍳','👨‍🍳','👩‍🍳','🧑‍🍹','👨‍🍹','👩‍🍹','☕','🧋','🧑‍💼','👩‍💼','👨‍💼','👑','👤','🙂']
const DIVISI_OPTIONS = ['Kitchen', 'Barista', 'Bar', 'All']

function KelolaUserView({ showToast, logAudit }) {
  const [users, setUsers] = useState([])
  const [loading, setLoading] = useState(true)
  const [form, setForm] = useState(null)      // null = tutup; object = modal terbuka
  const [saving, setSaving] = useState(false)

  const load = async () => {
    setLoading(true)
    const { data } = await supabase.from('app_users').select('*')
      .order('urutan', { ascending: true }).order('nama', { ascending: true })
    setUsers(data || [])
    setLoading(false)
  }
  useEffect(() => { load() }, [])
  ...
  const save = async () => {
    ...
    try {
      const isNew = !form.id
      const id = form.id || ((form.nama.toLowerCase().replace(/[^a-z0-9]/g, '') || 'user') + '_' + generateId().slice(0, 5))
      const row = {
  ...
  return (
    <div>
      ...
      {loading ? (
        <div style={{ textAlign: 'center', padding: '30px', color: C.text3, fontSize: '13px' }}>Memuat...</div>
      ) : (
        users.map(u => (
          <div key={u.id} style={{ background: C.panel, border: `1px solid ${C.border}`, borderRadius: '12px', padding: '12px 14px', marginBottom: '9px', display: 'flex', alignItems: 'center', gap: '12px' }}>
            <div style={{ fontSize: '26px' }}>{u.avatar}</div>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: '14px', fontWeight: 600, color: C.text, display: 'flex', alignItems: 'center', gap: '6px', flexWrap: 'wrap' }}>
                {u.nama}
                <span style={{ fontSize: '9px', fontWeight: 700, padding: '2px 7px', borderRadius: '20px', textTransform: 'uppercase',
                  background: u.role === 'owner' ? C.yellowBg : C.panel2,
            ...
            <div style={{ display: 'flex', gap: '6px' }}>
              <button onClick={() => openEdit(u)} style={{ width: '34px', height: '34px', borderRadius: '8px', border: `1px solid ${C.border}`, background: C.panel2, fontSize: '14px', cursor: 'pointer' }}>✏️</button>
              <button onClick={() => del(u)} style={{ width: '34px', height: '34px', borderRadius: '8px', border: `1px solid ${C.redBorder}`, background: C.redBg, fontSize: '14px', cursor: 'pointer' }}>🗑️</button>
            </div>
          </div>
        ))
      )}
      ...
      {form && (
        <div onClick={() => setForm(null)}
          style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px', zIndex: 50 }}>
          <div onClick={e => e.stopPropagation()}
            style={{ background: C.panel, borderRadius: '16px', padding: '22px', width: '100%', maxWidth: '360px', maxHeight: '90vh', overflow: 'auto' }}>
            <h3 style={{ fontSize: '16px', fontWeight: 700, color: C.text, marginBottom: '16px' }}>{form.id ? 'Edit User' : 'Tambah User'}</h3>

            <div style={{ marginBottom: '13px' }}>
              <label style={S.label}>Nama</label>
              <input style={S.input} value={form.nama} onChange={e => setForm({ ...form, nama: e.target.value })} placeholder="cth: Barista 2" />
            </div>
              ...
              <div style={{ display: 'flex', gap: '8px' }}>
                {['staff', 'owner'].map(r => (
                  <button key={r} onClick={() => setForm({ ...form, role: r })}
                    style={{ flex: 1, padding: '9px', borderRadius: '8px', fontSize: '12px', fontWeight: 600, cursor: 'pointer', textTransform: 'capitalize',
                      border: `1.5px solid ${form.role === r ? C.text : C.border}`,
                      background: form.role === r ? C.text : C.bg,
                      color: form.role === r ? C.panel : C.text2 }}>{r}</button>
                ))}
              </div>
            </div>
            ...
            <div style={{ marginBottom: '13px' }}>
              ...
              <select style={S.input} value={form.divisi} onChange={e => setForm({ ...form, divisi: e.target.value })}>
            ...
            <div style={{ marginBottom: '16px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: '5px' }}>
                {AVATAR_OPTIONS.map(em => (
                  <button key={em} onClick={() => setForm({ ...form, avatar: em })}
                    style={{ fontSize: '20px', padding: '6px 0', borderRadius: '8px', cursor: 'pointer', background: C.bg,
                      border: `1.5px solid ${form.avatar === em ? C.yellowBorder : 'transparent'}` }}>{em}</button>
                ))}
              </div>
            </div>

            <div style={{ display: 'flex', gap: '9px' }}>
              <button onClick={() => setForm(null)} style={{ ...S.btn, ...S.btnSecondary, flex: 1, padding: '12px' }}>Batal</button>
              <button onClick={save} disabled={saving} style={{ ...S.btn, ...S.btnSuccess, flex: 1, padding: '12px', opacity: saving ? 0.6 : 1 }}>{saving ? 'Menyimpan...' : '💾 Simpan'}</button>
            </div>
          </div>
        </div>
      )}
...
export default function App() {
  ...
  useEffect(() => {
    ...
    // Selalu muat belanja lengkap saat tab historybelanja dibuka
    if (view === 'historybelanja' && !lazyLoaded.historybelanja) {
      loadLazy('belanja_full', 'historybelanja')
    }
  }, [view, role])
...
function ProduksiView({ bahanBaku, showToast, loadData, logAudit, setView, userName, setUserName, isLocked }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      let fotoUrl = ''
      if (fotoFile) {
        fotoUrl = await uploadFotoToStorage(fotoFile, 'produksi')
      }
      const { error: prodErr } = await supabase.from('produksi').insert({
        id: newId, tanggal,
        ...
        foto: fotoUrl, status, yang_masak: yangMasak, catatan,
      })
      if (prodErr) throw new Error('Gagal simpan produksi: ' + prodErr.message)
...
function StokListView({ bahanBaku, showToast, loadData, logAudit, userName }) {
  ...
  const filtered = bahanBaku.filter(b => {
    ...
    const isSedang   = !isRendah && b.stok_saat_ini < b.stok_minimum * 1.5
    const isExpiring = (() => { const d = daysFromNow(b.expired_terdekat); return d !== null && d <= 2 && d >= 0 })()
    const statusOk =
      filterStatus === 'all'      ? true :
      filterStatus === 'rendah'   ? isRendah :
      filterStatus === 'sedang'   ? isSedang :
      filterStatus === 'expiring' ? isExpiring : true
    return katOk && divOk && searchOk && statusOk
  })

  const stats = {
    total:    bahanBaku.filter(b => b.is_active).length,
    rendah:   bahanBaku.filter(b => b.is_active && b.stok_saat_ini < b.stok_minimum).length,
    sedang:   bahanBaku.filter(b => b.is_active && b.stok_saat_ini >= b.stok_minimum && b.stok_saat_ini < b.stok_minimum * 1.5).length,
    expiring: bahanBaku.filter(b => { const d = daysFromNow(b.expired_terdekat); return b.is_active && d !== null && d <= 2 && d >= 0 }).length,
  }
  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '6px', marginBottom: '12px' }}>
        {[
          { key: 'all',      label: 'Total',    value: stats.total,    color: 'default' },
          { key: 'rendah',   label: 'Rendah',   value: stats.rendah,   color: 'red'     },
          { key: 'sedang',   label: 'Sedang',   value: stats.sedang,   color: 'yellow'  },
          { key: 'expiring', label: 'Expiring', value: stats.expiring, color: 'red'     },
        ].map(({ key, label, value, color }) => {
          const isActive = filterStatus === key
          const bg   = color === 'red'     ? C.redBg    : color === 'yellow' ? C.yellowBg : C.panel2
          const fg   = color === 'red'     ? C.red      : color === 'yellow' ? C.yellow   : C.text
          const bord = color === 'red'     ? C.redBorder: color === 'yellow' ? C.yellowBorder : C.border
          return (
            <div key={key} onClick={() => setFilterStatus(isActive ? 'all' : key)}
              style={{
                background: isActive ? (color === 'red' ? C.red : color === 'yellow' ? C.yellow : C.text) : bg,
                border: `1.5px solid ${isActive ? 'transparent' : bord}`,
                borderRadius: '8px', padding: '8px 6px', textAlign: 'center',
                cursor: 'pointer', transition: 'all .15s',
              }}>
              <div style={{ fontSize: '20px', fontWeight: 700, color: isActive ? '#fff' : fg }}>{value}</div>
              <div style={{ fontSize: '10px', color: isActive ? 'rgba(255,255,255,0.85)' : C.text3, marginTop: '1px' }}>{label}</div>
            </div>
          )
        })}
      </div>
      {filterStatus !== 'all' && (
        <div style={{ fontSize: '11px', color: C.text3, marginBottom: '8px', display: 'flex', alignItems: 'center', gap: '6px' }}>
          Filter aktif: <strong>{filterStatus}</strong>
          <button onClick={() => setFilterStatus('all')} style={{ fontSize: '11px', padding: '2px 8px', borderRadius: '99px', border: `1px solid ${C.border}`, background: 'transparent', cursor: 'pointer', color: C.text3 }}>✕ Reset</button>
        </div>
      )}
...
function WasteView({ bahanBaku, waste, showToast, loadData, logAudit, setView, userName, setUserName }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      let fotoUrl = ''
      if (fotoFile) {
        fotoUrl = await uploadFotoToStorage(fotoFile, 'waste')
      }
      const { error: wasteErr } = await supabase.from('waste').insert({
        id: newId, tanggal: formatTanggal(), bahan_id: bahan.id,
        jumlah: Number(jumlah), alasan, catatan, foto: fotoUrl,
        yang_catat: yangCatat,
      })
      if (wasteErr) throw new Error('Gagal simpan waste: ' + wasteErr.message)
      await supabase.from('bahan_baku').update({
...
function ResepView({ role, bahanBaku, showToast, userName }) {
  ...
  // Staff bisa tambah, tapi hanya owner yang bisa edit
  const canAdd  = true          // semua role bisa tambah resep baru
  const canEdit = role === 'owner'

  const fetchResep = async () => {
    setLoadingResep(true)
    const { data, error } = await supabase.from('resep').select('*').order('menu_nama')
    if (error) {
      console.error('fetchResep error:', error)
      showToast('❌ Gagal muat resep: ' + error.message)
    }
    setResepList(data || [])
  ...
  const filtered = resepList.filter(r =>
    r.menu_nama.toLowerCase().includes(search.toLowerCase())
  )

  // HPP pakai harga terbaru dari master, fallback ke snapshot saat input
  ...
  if (mode === 'form') return (
    <ResepForm
      ...
      canEdit={canEdit}
      onSave={async () => {
        // Paksa fetch ulang dari Supabase
        setLoadingResep(true)
        const { data, error } = await supabase.from('resep').select('*').order('menu_nama')
        if (error) showToast('❌ Gagal muat: ' + error.message)
        setResepList(data || [])
        setLoadingResep(false)
        setMode('list')
        setSelected(null)
  ...
  if (mode === 'view' && selected) {
    const totalHPP = hitungHPP(selected.bahan_list)
    const hppPerPorsi = selected.yield_qty > 0 ? totalHPP / selected.yield_qty : totalHPP
    return (
      <div>
        <div style={{ display: 'flex', gap: '8px', marginBottom: '14px' }}>
          ...
          {canEdit && (
            <button onClick={() => setMode('form')}
              style={{ ...S.btn, fontSize: '12px', padding: '7px 12px', background: C.yellowBg, color: C.yellow, border: `1px solid ${C.yellowBorder}` }}>
              ✏️ Edit resep
            </button>
          )}
        </div>
        ...
        {/* Header resep */}
        <div style={{ background: C.panel, border: `1px solid ${C.border}`, borderRadius: '12px', padding: '14px 16px', marginBottom: '12px' }}>
          <div style={{ fontSize: '17px', fontWeight: 700, color: C.text, marginBottom: '4px' }}>{selected.menu_nama}</div>
          <div style={{ fontSize: '12px', color: C.text3, marginBottom: '12px' }}>
            Batch untuk <strong>{selected.yield_qty} {selected.yield_satuan}</strong> · dibuat oleh {selected.created_by || '-'}
          </div>

          {/* COGS cards */}
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
            <div style={{ background: C.redBg, border: `1px solid ${C.redBorder}`, borderRadius: '8px', padding: '10px 12px' }}>
              <div style={{ fontSize: '10px', color: C.text3, marginBottom: '2px' }}>HPP total batch</div>
              <div style={{ fontSize: '17px', fontWeight: 700, color: C.red }}>{formatRupiah(Math.round(totalHPP))}</div>
              <div style={{ fontSize: '11px', color: C.text3, marginTop: '2px' }}>untuk {selected.yield_qty} {selected.yield_satuan}</div>
            </div>
            <div style={{ background: '#FFF3F3', border: `1px solid ${C.redBorder}`, borderRadius: '8px', padding: '10px 12px' }}>
              <div style={{ fontSize: '10px', color: C.text3, marginBottom: '2px' }}>COGS per porsi</div>
              <div style={{ fontSize: '17px', fontWeight: 700, color: C.red }}>{formatRupiah(Math.round(hppPerPorsi))}</div>
              <div style={{ fontSize: '11px', color: C.text3, marginTop: '2px' }}>per {selected.yield_satuan}</div>
            </div>
          </div>

          {selected.catatan && (
            <div style={{ fontSize: '12px', color: C.text3, marginTop: '10px', fontStyle: 'italic', background: C.panel2, padding: '8px 10px', borderRadius: '7px' }}>
              📝 {selected.catatan}
            </div>
          )}
        </div>
        ...
        {/* Daftar bahan */}
        <div style={{ fontSize: '12px', fontWeight: 600, color: C.text3, marginBottom: '8px' }}>🥬 Komposisi bahan:</div>
        {(selected.bahan_list || []).map((item, i) => {
          const master = bahanBaku.find(b => b.id === item.bahan_id)
          const namaDisplay = master?.nama || item.nama_snapshot || 'Bahan dihapus'
          const harga = master?.harga_per_satuan || item.harga_snapshot || 0
          const subtotal = Math.round((Number(item.jumlah) || 0) * harga)
          const hargaChange = master && item.harga_snapshot > 0 && master.harga_per_satuan !== item.harga_snapshot
          return (
            <div key={i} style={{
              background: C.panel, border: `1px solid ${C.border}`,
              borderRadius: '9px', padding: '10px 14px', marginBottom: '7px',
              display: 'flex', justifyContent: 'space-between', alignItems: 'center',
            }}>
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: '13px', fontWeight: 500, color: master ? C.text : C.text3 }}>{namaDisplay}</div>
                <div style={{ fontSize: '11px', color: C.text3, marginTop: '1px' }}>
                  {item.jumlah} {item.satuan || master?.satuan_dasar || ''} × {formatRupiah(harga)}
                  {hargaChange && (
                    <span style={{ marginLeft: '5px', fontSize: '10px', color: harga > item.harga_snapshot ? C.red : C.green }}>
                      {harga > item.harga_snapshot ? '↑' : '↓'} harga berubah
                    </span>
                  )}
                </div>
              </div>
              <div style={{ fontSize: '13px', fontWeight: 700, color: C.text }}>{formatRupiah(subtotal)}</div>
            </div>
          )
        })}

        {/* Total */}
        <div style={{ background: C.redBg, border: `1px solid ${C.redBorder}`, borderRadius: '8px', padding: '10px 14px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px' }}>
          <span style={{ fontSize: '13px', fontWeight: 600, color: C.red }}>Total COGS batch</span>
          <span style={{ fontSize: '15px', fontWeight: 700, color: C.red }}>{formatRupiah(Math.round(totalHPP))}</span>
        </div>
        <div style={{ background: C.panel2, borderRadius: '8px', padding: '10px 14px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <span style={{ fontSize: '13px', color: C.text2 }}>COGS per {selected.yield_satuan}</span>
          <span style={{ fontSize: '14px', fontWeight: 700, color: C.text }}>{formatRupiah(Math.round(hppPerPorsi))}</span>
        </div>
      </div>
    )
  }
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '14px' }}>
        <div>
          ...
          <p style={{ fontSize: '12px', color: C.text3, margin: 0 }}>
            {canEdit ? 'Kamu (owner) bisa tambah & edit resep' : 'Kamu bisa tambah resep baru · hanya owner yang bisa edit'}
          </p>
        </div>
        {canAdd && (
          <button onClick={() => { setSelected(null); setMode('form') }}
            style={{ ...S.btn, ...S.btnPrimary, padding: '8px 14px', fontSize: '12px', flexShrink: 0 }}>
            + Tambah
          </button>
        )}
      </div>
      ...
      <input value={search} onChange={e => setSearch(e.target.value)}
        placeholder="🔍 Cari nama menu..." style={{ ...S.input, marginBottom: '12px' }} />

      {loadingResep && (
        <div style={{ textAlign: 'center', padding: '24px', color: C.text3 }}>Memuat resep...</div>
      )}
      ...
      {!loadingResep && filtered.length === 0 && (
        <div style={{ textAlign: 'center', padding: '32px 20px', background: C.panel2, borderRadius: '12px' }}>
          ...
          <div style={{ fontSize: '14px', fontWeight: 500, marginBottom: '6px' }}>
            {resepList.length === 0 ? 'Belum ada resep' : 'Tidak ditemukan'}
          </div>
          <div style={{ fontSize: '12px', color: C.text3, lineHeight: 1.5 }}>
            Tap tombol <strong>+ Tambah</strong> untuk buat resep standar pertama.
          </div>
        </div>
      )}

      {filtered.map(r => {
        const totalHPP = hitungHPP(r.bahan_list)
        const hppPerPorsi = r.yield_qty > 0 ? totalHPP / r.yield_qty : totalHPP
        return (
          <div key={r.id} onClick={() => { setSelected(r); setMode('view') }}
            ...
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: '13px', fontWeight: 600, color: C.text }}>{r.menu_nama}</div>
              <div style={{ fontSize: '11px', color: C.text3, marginTop: '2px' }}>
                {(r.bahan_list || []).length} bahan · batch {r.yield_qty} {r.yield_satuan}
              </div>
            </div>
            <div style={{ textAlign: 'right', marginLeft: '12px', flexShrink: 0 }}>
              <div style={{ fontSize: '12px', color: C.text3 }}>COGS/porsi</div>
              <div style={{ fontSize: '14px', fontWeight: 700, color: C.red }}>{formatRupiah(Math.round(hppPerPorsi))}</div>
            </div>
          </div>
        )
      })}
...
function ResepForm({ initial, bahanBaku, userName, showToast, canEdit, onSave, onCancel }) {
  ...
  const [bahanList, setBahanList]   = useState(
    initial?.bahan_list?.length > 0
      ? initial.bahan_list.map(b => ({ ...b }))
  ...
  const updateBahan = (idx, field, val) =>
    setBahanList(prev => prev.map((b, i) => i === idx ? { ...b, [field]: val } : b))

  const pickBahan = (idx, bahanId) => {
    const master = bahanBaku.find(b => b.id === bahanId)
    setBahanList(prev => prev.map((b, i) => i === idx ? {
      ...b,
      bahan_id:       bahanId,
      satuan:         master?.satuan_dasar || '',
      harga_snapshot: master?.harga_per_satuan || 0,
  ...
  const totalHPP = bahanList.reduce((s, item) => {
    const master = bahanBaku.find(b => b.id === item.bahan_id)
    const harga = master?.harga_per_satuan || item.harga_snapshot || 0
    return s + (Number(item.jumlah) || 0) * harga
  }, 0)
  const hppPerPorsi = yieldQty > 0 ? totalHPP / Number(yieldQty) : totalHPP

  const handleSave = async () => {
    if (!menuNama.trim())                                     { showToast('❌ Isi nama menu dulu'); return }
    if (bahanList.length === 0)                               { showToast('❌ Tambah minimal 1 bahan'); return }
    if (bahanList.find(b => !b.bahan_id))                     { showToast('❌ Pilih bahan di semua baris'); return }
    if (bahanList.find(b => !b.jumlah || Number(b.jumlah) <= 0)) { showToast('❌ Isi jumlah di semua bahan'); return }

    setSaving(true)
    const payload = {
      menu_nama:    menuNama.trim(),
      yield_qty:    Number(yieldQty) || 1,
      yield_satuan: yieldSatuan || 'porsi',
      harga_jual:   0,
      ...
      bahan_list:   bahanList.map(b => {
        ...
        return {
          ...
          satuan:         b.satuan || master?.satuan_dasar || '',
          harga_snapshot: master?.harga_per_satuan || 0,
          nama_snapshot:  master?.nama || b.nama_snapshot || '',
        }
      }),
    }
    ...
    let result
    if (isEdit) {
      result = await supabase.from('resep').update(payload).eq('id', initial.id)
    } else {
      result = await supabase.from('resep').insert({ ...payload, id: generateId() })
    }

    setSaving(false)

    if (result.error) {
      showToast('❌ Gagal simpan: ' + result.error.message)
      return
    }
  ...
  const handleDelete = async () => {
    if (!canEdit) return
    setDeleting(true)
    const result = await supabase.from('resep').delete().eq('id', initial.id)
    setDeleting(false)
  ...
  // Semua bahan bisa dipilih (mentah, prepack, jadi) — chef knows best
  const bahanOptions = bahanBaku.map(b => ({
    value: b.id, label: b.nama, stock: b.stok_saat_ini, satuan: b.satuan_dasar,
  }))
    ...
    <div>
      <div style={{ display: 'flex', gap: '8px', marginBottom: '14px', alignItems: 'center' }}>
        ...
        <h2 style={{ fontSize: '16px', fontWeight: 600, margin: 0 }}>
          {isEdit ? '✏️ Edit Resep' : '📖 Resep Baru'}
        </h2>
      </div>

      <FormRow label="Nama menu">
        <input value={menuNama} onChange={e => setMenuNama(e.target.value)}
          placeholder="Contoh: Ayam Bakar Bumbu Rempah" style={S.input}
          readOnly={isEdit && !canEdit} />
      </FormRow>

      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
        <FormRow label="Batch untuk berapa porsi">
          <input type="number" value={yieldQty} onChange={e => setYieldQty(e.target.value)}
            style={S.input} min="1" step="1"
            readOnly={isEdit && !canEdit} />
        </FormRow>
        <FormRow label="Satuan">
          <select value={yieldSatuan} onChange={e => setYieldSatuan(e.target.value)}
            style={S.input} disabled={isEdit && !canEdit}>
            <option>porsi</option><option>pcs</option><option>gram</option>
            <option>liter</option><option>batch</option><option>ekor</option>
          </select>
        </FormRow>
      </div>

      <div style={{ fontSize: '12px', fontWeight: 600, color: C.text3, margin: '12px 0 8px' }}>
        🥬 Bahan-bahan yang dipakai:
      </div>

      {bahanList.map((item, idx) => {
        const master = bahanBaku.find(b => b.id === item.bahan_id)
        const subtotal = Math.round((Number(item.jumlah) || 0) * (master?.harga_per_satuan || item.harga_snapshot || 0))
        return (
          <div key={idx} style={{
            background: C.panel, border: `1px solid ${C.border}`,
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '7px' }}>
              ...
              {(!isEdit || canEdit) && bahanList.length > 1 && (
                <button onClick={() => removeBahan(idx)} style={{
                  padding: '2px 7px', fontSize: '11px', borderRadius: '4px',
                  background: C.redBg, color: C.red, border: `1px solid ${C.redBorder}`, cursor: 'pointer',
                }}>✕</button>
              )}
            </div>
            <SearchableSelect
              ...
              value={item.bahan_id}
              onChange={val => (!isEdit || canEdit) && pickBahan(idx, val)}
              placeholder="Pilih bahan (mentah / bumbu / prepack)..."
              showStock={false}
            />
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '6px', marginTop: '7px' }}>
              <div>
                <div style={{ fontSize: '10px', color: C.text3, marginBottom: '3px' }}>Jumlah</div>
                <input type="number" value={item.jumlah}
                  onChange={e => (!isEdit || canEdit) && updateBahan(idx, 'jumlah', e.target.value)}
                  placeholder="0" style={S.input} step="0.1"
                  readOnly={isEdit && !canEdit} />
              </div>
              <div>
                <div style={{ fontSize: '10px', color: C.text3, marginBottom: '3px' }}>Satuan</div>
                <input type="text"
                  value={item.satuan || master?.satuan_dasar || ''}
                  onChange={e => (!isEdit || canEdit) && updateBahan(idx, 'satuan', e.target.value)}
                  placeholder="gr/ml/pcs" style={S.input}
                  readOnly={isEdit && !canEdit} />
              </div>
            </div>
            {master && item.jumlah && (
              <div style={{ fontSize: '11px', color: C.text3, marginTop: '4px' }}>
                ...
                {!master.harga_per_satuan && (
                  <span style={{ color: C.yellow, marginLeft: '4px' }}>⚠️ Harga belum diset</span>
                )}
              </div>
            )}
          </div>
      ...
      {(!isEdit || canEdit) && (
        <button onClick={addBahan} style={{
          width: '100%', padding: '9px', fontSize: '12px', borderRadius: '8px',
          border: `1.5px dashed ${C.border}`, background: 'transparent',
          color: C.text3, cursor: 'pointer', marginBottom: '12px',
        }}>+ Tambah bahan</button>
      )}

      {/* Preview COGS */}
      {totalHPP > 0 && (
        <div style={{ background: C.redBg, border: `1px solid ${C.redBorder}`, borderRadius: '8px', padding: '10px 14px', marginBottom: '10px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '4px' }}>
            <span style={{ fontSize: '12px', color: C.text3 }}>Total COGS batch ({yieldQty} {yieldSatuan})</span>
            <span style={{ fontSize: '14px', fontWeight: 700, color: C.red }}>{formatRupiah(Math.round(totalHPP))}</span>
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            <span style={{ fontSize: '12px', color: C.text3 }}>COGS per {yieldSatuan}</span>
            <span style={{ fontSize: '14px', fontWeight: 700, color: C.red }}>{formatRupiah(Math.round(hppPerPorsi))}</span>
          </div>
        </div>
      )}
      ...
      {(!isEdit || canEdit) && (
        <button onClick={handleSave} disabled={saving} style={{
          ...S.btn, ...S.btnPrimary, width: '100%', padding: '13px',
          marginBottom: isEdit && canEdit ? '8px' : '0',
          opacity: saving ? 0.6 : 1,
        }}>
          {saving ? 'Menyimpan...' : (isEdit ? '✅ Update resep' : '✅ Simpan resep')}
        </button>
      )}

      {isEdit && canEdit && (
        <button onClick={handleDelete} disabled={deleting} style={{
          ...S.btn, ...S.btnDanger, width: '100%', padding: '10px',
      ...
      {isEdit && !canEdit && (
        <div style={{ fontSize: '11px', color: C.text3, textAlign: 'center', padding: '8px', background: C.panel2, borderRadius: '7px' }}>
          🔒 Hanya owner yang bisa edit resep yang sudah ada
        </div>
      )}
    </div>
  )
