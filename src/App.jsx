...
export default function App() {
  ...
  // Lazy load flags — data hanya dimuat saat tab dibuka pertama kali
  ...
  // Tab berubah → muat data lazy jika belum pernah dimuat
  useEffect(() => {
    if (!role) return
    const lazyTabs = {
      waste:          () => loadLazy('waste'),
      historybelanja: () => loadLazy('belanja_full'),
      auditlog:       () => loadLazy('audit'),
      dashboard:      () => loadLazy('audit'),
    }
    if (lazyTabs[view] && !lazyLoaded[view]) lazyTabs[view]()
  }, [view, role])

  const showToast = (msg) => { setToast(msg); setTimeout(() => setToast(''), 2400) }

  // ─── Data kritikal: dimuat saat pertama login ───
  // Hanya yang dibutuhkan di Home, Produksi, Stok, Closing
  ...
  // ─── Data lazy: dimuat on-demand saat tab dibuka ───
  const loadLazy = async (type) => {
    try {
      if (type === 'waste' && !lazyLoaded.waste) {
        const { data } = await supabase.from('waste').select('*').order('created_at', { ascending: false }).limit(100)
        setWaste(data || [])
        setLazyLoaded(p => ({ ...p, waste: true, [view]: true }))
      }
      if (type === 'belanja_full' && !lazyLoaded.historybelanja) {
        // Muat lebih banyak belanja untuk history view
        const { data } = await supabase.from('belanja').select('*').order('created_at', { ascending: false }).limit(200)
        setBelanja(data || [])
        setLazyLoaded(p => ({ ...p, historybelanja: true }))
      }
      if (type === 'audit' && !lazyLoaded.audit) {
        const { data } = await supabase.from('audit_log').select('*').order('created_at', { ascending: false }).limit(30)
        setAuditLog(data || [])
        setLazyLoaded(p => ({ ...p, audit: true, auditlog: true, dashboard: true }))
      }
    } catch (err) { console.error('Lazy load error:', err.message) }
  }

  // loadData tetap ada untuk refresh manual setelah input
  const loadData = async () => {
    await loadDataCritical()
    // Refresh data lazy yang sudah pernah dimuat
    if (lazyLoaded.waste) {
      const { data } = await supabase.from('waste').select('*').order('created_at', { ascending: false }).limit(100)
      setWaste(data || [])
    }
    ...
    <div style={{ position: 'fixed', inset: 0, background: C.bg, display: 'flex', flexDirection: 'column', zIndex: 999, padding: '0' }}>
      ...
      <div style={{ padding: '18px 20px', flex: 1 }}>
        ...
        <div className="skeleton-item" style={{ textAlign: 'center', paddingTop: '8px' }}>
          <span style={{ fontSize: '11px', color: C.text3 }}>☕ Menyiapkan data Piccolo Corner...</span>
        </div>
      </div>
    </div>
  )
...
function HistoryBelanjaView({ belanja, showToast, loadData }) {
  ...
  const saveEdit = async () => {
    setSaving(true)
    try {
      await supabase.from('belanja').update({
        jalur: editJalur,
        sumber_dana: editSumber,
        catatan: editCatatan,
      }).eq('id', selected.id)
      showToast('✅ Nota berhasil diupdate')
      await loadData()
      // Update selected locally so UI reflects change immediately
      setSelected(prev => ({ ...prev, jalur: editJalur, sumber_dana: editSumber, catatan: editCatatan }))
      setEditing(false)
    } catch (e) { showToast('❌ ' + e.message) }
    setSaving(false)
  }
      ...
      <div>
        <div style={{ display: 'flex', gap: '8px', marginBottom: '14px' }}>
          <button onClick={() => { setSelected(null); setEditing(false) }}
            style={{ ...S.btn, ...S.btnSecondary, fontSize: '12px', padding: '7px 12px' }}>
            ← Kembali
          </button>
          {!editing && (
        ...
        {editing ? (
          <div style={{ background: C.panel, border: `1.5px solid ${C.yellowBorder}`, borderRadius: '12px', padding: '14px 16px', marginBottom: '12px' }}>
            <div style={{ fontSize: '13px', fontWeight: 600, color: C.yellow, marginBottom: '12px' }}>✏️ Edit detail nota</div>
            ...
            <label style={S.label}>Catatan</label>
            <textarea rows={2} value={editCatatan} onChange={e => setEditCatatan(e.target.value)} style={{ ...S.input, marginBottom: '12px' }} placeholder="Catatan tambahan..." />

            <div style={{ background: C.yellowBg, border: `1px solid ${C.yellowBorder}`, borderRadius: '7px', padding: '8px 12px', marginBottom: '12px', fontSize: '11px', color: C.yellow }}>
              ⚠️ Hanya jalur, sumber dana, dan catatan yang bisa diubah. Item belanja dan total tidak berubah.
            </div>

            <div style={{ display: 'flex', gap: '8px' }}>
              <button onClick={() => setEditing(false)} style={{ ...S.btn, ...S.btnSecondary, flex: 1 }}>Batal</button>
              <button onClick={saveEdit} disabled={saving}
                style={{ ...S.btn, ...S.btnPrimary, flex: 2, opacity: saving ? 0.6 : 1 }}>
                {saving ? 'Menyimpan...' : '✅ Simpan perubahan'}
              </button>
            </div>
          </div>
        ) : (
          /* Header nota — view mode */
          <div style={{ background: C.panel, border: `1px solid ${C.border}`, borderRadius: '12px', padding: '14px 16px', marginBottom: '12px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '10px' }}>
              <div>
                <div style={{ fontSize: '15px', fontWeight: 600, color: C.text }}>🧾 Nota Belanja</div>
                <div style={{ fontSize: '12px', color: C.text3, marginTop: '2px' }}>
                  {formatTanggalID(selected.tanggal)} · oleh <strong>{selected.yang_belanja || '-'}</strong>
                </div>
              </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginBottom: selected.catatan ? '10px' : '0' }}>
              <div style={{ background: C.panel2, borderRadius: '8px', padding: '8px 10px' }}>
                <div style={{ fontSize: '10px', color: C.text3 }}>Total belanja</div>
                <div style={{ fontSize: '16px', fontWeight: 700, color: C.green }}>{formatRupiah(selected.total_harga)}</div>
              </div>
              <div style={{ background: C.panel2, borderRadius: '8px', padding: '8px 10px' }}>
                <div style={{ fontSize: '10px', color: C.text3 }}>Sumber dana</div>
                <div style={{ fontSize: '13px', fontWeight: 600, color: C.text }}>{selected.sumber_dana?.replace('_', ' ') || '-'}</div>
              </div>
            </div>
            {selected.catatan && (
              <div style={{ fontSize: '12px', color: C.text3, fontStyle: 'italic' }}>📝 {selected.catatan}</div>
            )}
          </div>
        )}

        {/* Daftar item — selalu tampil */}
        {!editing && (
          <>
            <div style={{ fontSize: '12px', fontWeight: 600, color: C.text3, marginBottom: '8px' }}>
              🛍️ {items.length} item yang dibeli:
            </div>
            {items.map((item, i) => {
              const hargaPerSatuan = item.jumlah > 0 ? Math.round(item.harga / item.jumlah) : 0
              return (
                <div key={i} style={{
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: '11px', color: C.text3, marginTop: '2px' }}>
                      ...
                      {hargaPerSatuan > 0 && (
                        <span style={{ marginLeft: '6px', background: C.greenLightBg, color: C.greenLight, fontSize: '10px', padding: '1px 6px', borderRadius: '99px' }}>
                          {formatRupiah(hargaPerSatuan)}/{item.satuan}
                        </span>
                      )}
                    </div>
                    {item.tanggal_expired && (
                      <div style={{ fontSize: '10px', color: C.yellow, marginTop: '2px' }}>⏰ Exp: {formatTanggalID(item.tanggal_expired)}</div>
                    )}
                  </div>
                  <div style={{ fontSize: '14px', fontWeight: 700, color: C.text, marginLeft: '10px' }}>
            ...
            {selected.foto_nota && (
              <div style={{ marginTop: '4px' }}>
                ...
                <img src={selected.foto_nota} alt="foto nota" onClick={() => setFotoModal(selected.foto_nota)}
                  style={{ width: '100%', maxWidth: '300px', borderRadius: '10px', border: `1px solid ${C.border}`, cursor: 'pointer' }} />
                <div style={{ fontSize: '10px', color: C.text3, marginTop: '4px' }}>Tap foto untuk perbesar</div>
              </div>
            )}
          </>
        )}
        ...
        {fotoModal && (
          <div onClick={() => setFotoModal(null)} style={{
            ...
            <div style={{ position: 'absolute', top: '20px', right: '20px', color: '#fff', fontSize: '28px', cursor: 'pointer', fontWeight: 300 }}>✕</div>
          </div>
        )}
      </div>
    )
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: '12px', color: C.text3, marginBottom: '14px' }}>Tap nota untuk lihat detail item · {filtered.length} transaksi</p>
      ...
      {filtered.length === 0 && (
        <div style={{ textAlign: 'center', padding: '24px', color: C.text3, fontSize: '13px' }}>
          Belum ada belanja di periode ini
        </div>
      )}
        ...
        <div key={b.id} onClick={() => setSelected(b)} style={{
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: '13px', fontWeight: 600, color: C.text, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
              {b.items?.map(i => i.nama).join(', ') || 'Belanja'}
            </div>
            <div style={{ fontSize: '11px', color: C.text3, marginTop: '3px', display: 'flex', gap: '6px', alignItems: 'center', flexWrap: 'wrap' }}>
              ...
              <span>·</span>
              <span>{b.yang_belanja || '-'}</span>
              <span>·</span>
              <span>{b.items?.length || 0} item</span>
            </div>
          </div>
...
// Komponen belanja tab untuk Owner Dashboard (sama dengan staff tapi tanpa header h2)
function BelanjaTabOwner({ belanja }) {
  const [filter, setFilter] = useState('month')
  const [selected, setSelected] = useState(null)
  const [fotoModal, setFotoModal] = useState(null)
  const now = new Date()
    ...
    return (
      <div>
        <button onClick={() => setSelected(null)}
          style={{ ...S.btn, ...S.btnSecondary, fontSize: '12px', marginBottom: '12px', padding: '7px 12px' }}>
          ← Kembali ke daftar
        </button>
        <div style={{ background: C.panel, border: `1px solid ${C.border}`, borderRadius: '12px', padding: '14px 16px', marginBottom: '12px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '10px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginBottom: selected.catatan ? '8px' : '0' }}>
            <div style={{ background: C.panel2, borderRadius: '7px', padding: '7px 10px' }}>
              <div style={{ fontSize: '10px', color: C.text3 }}>Total</div>
              <div style={{ fontSize: '15px', fontWeight: 700, color: C.green }}>{formatRupiah(selected.total_harga)}</div>
            </div>
            <div style={{ background: C.panel2, borderRadius: '7px', padding: '7px 10px' }}>
              <div style={{ fontSize: '10px', color: C.text3 }}>Sumber dana</div>
              <div style={{ fontSize: '12px', fontWeight: 600 }}>{selected.sumber_dana?.replace('_', ' ') || '-'}</div>
            </div>
          </div>
          {selected.catatan && <div style={{ fontSize: '12px', color: C.text3, fontStyle: 'italic' }}>📝 {selected.catatan}</div>}
        </div>
        {items.map((item, i) => {
          const hpsSatuan = item.jumlah > 0 ? Math.round(item.harga / item.jumlah) : 0
          return (
            <div key={i} style={{ background: C.panel, border: `1px solid ${C.border}`, borderRadius: '10px', padding: '11px 14px', marginBottom: '7px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontSize: '11px', color: C.text3, marginTop: '2px' }}>
                  {item.jumlah} {item.satuan}
                  {hpsSatuan > 0 && <span style={{ marginLeft: '6px', background: C.greenLightBg, color: C.greenLight, fontSize: '10px', padding: '1px 6px', borderRadius: '99px' }}>{formatRupiah(hpsSatuan)}/{item.satuan}</span>}
                </div>
              </div>
        ...
        {selected.foto_nota && (
          <div style={{ marginTop: '8px' }}>
            <div style={{ fontSize: '11px', color: C.text3, marginBottom: '6px' }}>📷 Foto nota:</div>
            <img src={selected.foto_nota} alt="nota" onClick={() => setFotoModal(selected.foto_nota)}
              style={{ width: '100%', maxWidth: '280px', borderRadius: '8px', border: `1px solid ${C.border}`, cursor: 'pointer' }} />
          </div>
        )}
        {fotoModal && (
  ...
  return (
    <div>
      ...
      <div style={{ background: C.greenBg, border: `1px solid ${C.greenBorder}`, padding: '7px 12px', borderRadius: '7px', fontSize: '12px', marginBottom: '10px' }}>
        💰 <strong style={{ color: C.green }}>{formatRupiah(total)}</strong> · {filtered.length} transaksi · tap untuk detail
      </div>
      {filtered.map(b => (
...
function OwnerDashboardView({ bahanBaku, produksi, belanja, closing, waste, auditLog, showToast }) {
  ...
  return (
    <div>
      ...
      {tab === 'belanja' && <BelanjaTabOwner belanja={belanja} showToast={showToast} />}
      ...
      {tab === 'waste' && (() => {
        ...
        // Hitung nilai Rp tiap waste pakai harga_per_satuan dari master
        ...
        const exportWaste = () => {
          const data = wasteWithValue.map(w => ({
            Tanggal: w.tanggal,
            'Nama Bahan': w.bahan_nama,
            Jumlah: w.jumlah,
            Satuan: w.satuan,
            'Harga/Satuan (Rp)': bahanBaku.find(x => x.id === w.bahan_id)?.harga_per_satuan || 0,
            'Nilai Kerugian (Rp)': w.nilai_rp,
            Alasan: w.alasan,
            'Yang Catat': w.yang_catat || '-',
            Catatan: w.catatan || '',
            'Ada Foto': w.foto ? 'Ya' : 'Tidak',
          }))
          const ws = XLSX.utils.json_to_sheet(data)
        ...
        return (
          <div>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
              <div style={{ display: 'flex', gap: '6px' }}>
                {[['today','Hari ini'],['week','Minggu ini'],['month','Bulan ini']].map(([k,l]) => (
                  <button key={k} onClick={() => setWasteFilter(k)} style={{
                    padding: '5px 10px', fontSize: '11px', borderRadius: '99px',
                    border: `1px solid ${C.border}`, cursor: 'pointer',
                    background: wasteFilter === k ? C.text : 'transparent',
                    color: wasteFilter === k ? C.panel : C.text2,
                  }}>{l}</button>
                ))}
              </div>
              <button onClick={exportWaste} style={{ ...S.btn, ...S.btnSecondary, fontSize: '11px', padding: '6px 10px' }}>
                📥 Export
              </button>
            </div>
            ...
            <div style={{ background: C.redBg, border: `1px solid ${C.redBorder}`, borderRadius: '10px', padding: '12px 14px', marginBottom: '12px' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '8px' }}>
                <div>
                  <div style={{ fontSize: '11px', color: C.text3 }}>Total kejadian waste</div>
                  <div style={{ fontSize: '22px', fontWeight: 700, color: C.red }}>{wasteWithValue.length}</div>
                </div>
                <div>
                  <div style={{ fontSize: '11px', color: C.text3 }}>Total nilai kerugian</div>
                  <div style={{ fontSize: '22px', fontWeight: 700, color: C.red }}>{formatRupiah(totalNilai)}</div>
                </div>
              </div>
              {topAlasan.length > 0 && (
                <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
                  {topAlasan.map(([alasan, count]) => (
                    <span key={alasan} style={{ fontSize: '11px', background: C.redBorder, color: C.red, padding: '2px 8px', borderRadius: '99px' }}>
                      {alasan}: {count}×
                    </span>
                  ))}
                </div>
            ...
            {wasteWithValue.length === 0 && (
              <div style={{ textAlign: 'center', padding: '24px', color: C.text3, fontSize: '13px' }}>
                ✅ Tidak ada waste di periode ini
              </div>
            )}
            ...
            {wasteWithValue.map(w => (
              <div key={w.id}>
                <div onClick={() => setSelectedWaste(selectedWaste?.id === w.id ? null : w)}
                  style={{ background: C.panel, border: `1px solid ${C.border}`, borderRadius: '10px', padding: '11px 14px', marginBottom: '6px', cursor: 'pointer' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: '13px', fontWeight: 600, color: C.text }}>{w.bahan_nama}</div>
                      <div style={{ fontSize: '11px', color: C.text3, marginTop: '2px' }}>
                        {formatTanggalID(w.tanggal)} · oleh {w.yang_catat || '-'}
                      </div>
                      <div style={{ display: 'flex', gap: '6px', marginTop: '4px', flexWrap: 'wrap' }}>
                        <span style={{ fontSize: '11px', background: C.yellowBg, color: C.yellow, padding: '2px 7px', borderRadius: '99px' }}>
                          {w.alasan}
                        </span>
                    ...
                    <div style={{ textAlign: 'right', marginLeft: '12px', flexShrink: 0 }}>
                      ...
                      {w.nilai_rp > 0 && (
                        <div style={{ fontSize: '12px', color: C.red, marginTop: '2px' }}>
                          {formatRupiah(w.nilai_rp)}
                        </div>
                      )}
                      {w.nilai_rp === 0 && (
                  ...
                  {selectedWaste?.id === w.id && (
                    <div style={{ marginTop: '10px', paddingTop: '10px', borderTop: `1px solid ${C.panel2}` }}>
                      ...
                      {w.foto && (
                        <img src={w.foto} alt="bukti waste"
                          style={{ width: '100%', maxWidth: '280px', borderRadius: '8px', border: `1px solid ${C.border}` }} />
                      )}
                      {!w.foto && (
