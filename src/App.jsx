    ...
    <div>
      ...
      {tab === 'produksi' && (
        <div>
          ...
          {produksi.slice(0, 15).map(p => {
            const cogs = p.total_cogs || 0
            const cogsPorsi = p.cogs_per_porsi || 0
            return (
              <div key={p.id} style={{ background: C.panel2, padding: '11px 12px', borderRadius: '8px', marginBottom: '6px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  <div>
                    ...
                    <div style={{ fontSize: '11px', color: C.text3 }}>
                      Hasil: {p.hasil_pcs} sat · {p.hasil_porsi} porsi
                    </div>
                  </div>
      ...
      {tab === 'hpp' && bahanBaku.filter(b => b.kategori !== 'mentah').slice(0, 10).map(b => (
        <div key={b.id} style={{ background: C.panel2, padding: '11px 12px', borderRadius: '8px', marginBottom: '6px' }}>
